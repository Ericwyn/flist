...
- **文件预览** — 文本 / Markdown / CSV / Excel / Word / PowerPoint / 图片 / 视频 / 音频 / PDF；文档在浏览器本地解析，图片支持完整浏览器操作与 EXIF 信息
- **在线编辑** — CodeMirror 6 语法高亮，乐观锁保存，冲突检测
- **文件操作** — 批量移动 / 复制 / 删除 / 重命名，支持自动避让
- **分片上传** — 断点续传（文件指纹），冲突处理（覆盖 / 改名 / 取消），实时进度
- **打包下载** — 多文件流式 zip，智能压缩
- **在线解压** — 右键后台解压 ZIP / RAR / TAR.GZ，支持进度、取消与失败回滚
- **搜索** — 递归搜索，超时保护，结果上限
- **收藏夹** — 目录收藏、拖拽排序、有效性标记
...
| 类别 | 格式 | 预览能力 |
|------|------|----------|
| Markdown | `.md` `.markdown` | 默认排版阅读，可切换为源码编辑 |
| 数据表格 | `.csv` | 自动识别分隔符并以冻结行列头的表格展示，可切换为文本编辑 |
| Excel | `.xls` `.xlsx` `.xlsm` `.xlsb` `.ods` | 多工作表浏览、单元格格式化展示 |
| Word | `.docx` | 分页、页眉页脚、表格、图片、批注和修订展示 |
| PowerPoint | `.pptx` | 浏览器原生幻灯片渲染，长文稿按需挂载 |

上述文档只下载到当前浏览器处理，不会上传第三方转换服务。服务端和前端均限制单个预览文件最大为 20 MiB；旧版二进制 `.doc` / `.ppt` 暂不支持在线渲染，可下载后查看或先转换为 `.docx` / `.pptx`。
